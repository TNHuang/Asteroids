(function () {
  if (typeof Asteroids === "undefined") {
    window.Asteroids = {};
  }

  function randomColor () {
    // var hexDigits = "0123456789ABCDEF";
    //
    // var color = "#";
    // for (var i = 0; i < 3; i ++) {
    //   color += hexDigits[Math.round((Math.random() * 16))];
    // }
    // return color;
    return "#fff";
  }

  var Ship = Asteroids.Ship = function (options) {
    options.radius = Ship.RADIUS;
    options.vel = options.vel || [0, 0];
    options.color = options.color || randomColor();
    options.dir = options.dir;
    Asteroids.MovingObject.call(this, options);

    // this.canvas = $("canvas");

    this.sprite = new Image();
    this.sprite.src = "assets/shuttle.png";
  };

  Ship.RADIUS = 15;
  Ship.SPEEDLIMIT = 10;

  Asteroids.Util.inherits(Ship, Asteroids.MovingObject);

  Ship.prototype.fireBullet = function () {
    var norm = Asteroids.Util.norm(this.vel);

    if (norm == 0) {
      // Can't fire unless moving.
      return;
    }

    var relVel = Asteroids.Util.scale(
      Asteroids.Util.dir(this.vel),
      Asteroids.Bullet.SPEED
    );

    var bulletVel = [
      relVel[0] + this.vel[0], relVel[1] + this.vel[1]
    ];

    var bullet = new Asteroids.Bullet({
      pos: this.pos,
      vel: bulletVel,
      color: this.color,
      game: this.game
    });

    this.game.add(bullet);
  };

  Ship.prototype.draw = function(ctx) {

    var newX = this.pos[0] - this.vel[0] * this.radius;
    var newY = this.pos[1] - this.vel[1] * this.radius;
    ctx.beginPath();
    ctx.moveTo(this.pos[0], this.pos[1]);
    ctx.lineTo(newX, newY);
    var gradient = ctx.createLinearGradient(0,0,40,0);
    gradient.addColorStop("0.5", "orange");
    // gradient.addColorStop("0.5", "red");
    ctx.strokeStyle = gradient;
    ctx.lineWidth = 5;
    ctx.shadowBlur = 20;
    ctx.shadowColor = "red";
    ctx.stroke();
    ctx.fill();

    ctx.save();
    ctx.translate(this.pos[0], this.pos[1]);
    // var offsetX = this.canvas.offsetLeft;
    // var offsetY = this.canvas.offsetTop;

    ctx.rotate( Math.PI - Asteroids.Util.angle(this.vel) );

    ctx.drawImage(this.sprite, -this.radius - 10
                  , -this.radius - 10, this.radius * 2 + 20
                  , this.radius * 2 + 30);
    ctx.restore();
  };

  Ship.prototype.power = function (impulse) {

    this.vel[0] += impulse[0];
    this.vel[1] += impulse[1];
    if ( Asteroids.Util.norm(this.vel) > Ship.SPEEDLIMIT ) {
      this.vel = Asteroids.Util.dir(this.vel);
      this.vel[0] *= Ship.SPEEDLIMIT;
      this.vel[1] *= Ship.SPEEDLIMIT;
    }
  };

  Ship.prototype.relocate = function () {
    this.pos = this.game.randomPosition();
    this.vel = [0, 0];
  };
})();
